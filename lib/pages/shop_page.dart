              ...
              itemBuilder: (context, index) {
                ...
                return Container(
                  decoration: BoxDecoration(
                    border: Border.all(
                      color: isEquipped ? const Color.fromARGB(255, 246, 184, 69) : Colors.black, // 👈 高亮
                      width: isEquipped ? 4 : 2,                        // 👈 變粗
                    ),
                    borderRadius: BorderRadius.circular(15),
                    color: isEquipped ? const Color.fromARGB(255, 249, 241, 135).withOpacity(0.1) : null, // 👈 淡底色（可選）
                  ),
                      ...
                      ElevatedButton(
                        onPressed: () {
                          if (!owned) {
                            ...
                            if (isEquipped) {
                              if (item["type"] == "rock") {
                                widget.onEquip("assets/rock/rock01.png", widget.equippedHat, widget.equippedEyes);
                              } else if (item["type"] == "hat") {
                                widget.onEquip(widget.equippedRock, null, widget.equippedEyes);
                              } else if (item["type"] == "eyes") {
                                widget.onEquip(widget.equippedRock, widget.equippedHat, null);
                              }
                              return;
                            }
                            ...
                            // 👉 裝備
                            if (item["type"] == "rock") {
                              widget.onEquip(item["image"], widget.equippedHat, widget.equippedEyes);
                            } else if (item["type"] == "hat") {
                              widget.onEquip(widget.equippedRock, item["image"], widget.equippedEyes);
                            } else if (item["type"] == "eyes") {
                              widget.onEquip(widget.equippedRock, widget.equippedHat, item["image"]);
                            }
                          }
                        },
                        child: Text(
